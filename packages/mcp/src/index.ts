...
const TOOLS = [
  ...
  },
  {
    ...
    description:
      ...
      'On an IOSS-treated calculation (EU B2C goods <= EUR150 imported under a seller IOSS registration), the response may also carry a top-level customsDuty object (sibling of ioss, absent whenever IOSS treatment was not applied to this calculation — Clearvo estimates the duty only for IOSS-treated consignments; the duty itself also applies to non-IOSS low-value imports, e.g. Special Arrangements or standard import VAT, which Clearvo does not yet model, so absence here is not evidence no duty is owed): an informational, EUR-denominated ESTIMATE of the EU\'s temporary per-distinct-tariff-classification customs duty on low-value consignments (2026-07-01 to 2028-06-30). It is includedInTotals: false and payableBy: \'DECLARANT\' — never added to totalTax/totalAmountWithTax, never collected from the buyer, never remitted by Clearvo; it is owed by the IOSS holder or their indirect customs representative. Passing commodityCode on each line item (its tariff/HS classification code) determines how many distinct duty items are counted — a line with no usable commodityCode still counts (flagged in linesMissingCommodityCode). On a credit note the duty is never reversed: customsDuty is absent and customsDutyNote is \'NOT_REVERSED_ON_CREDIT_NOTE\' instead. ' +
