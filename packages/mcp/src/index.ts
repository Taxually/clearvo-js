        ...
        lineItems: {
          ...
          items: {
            ...
            properties: {
              ...
              productName: { type: 'string', description: 'Product or service name — used for an exact-match lookup against your product catalogue if productCode is absent or unmatched. No live AI classification occurs on this endpoint; an unmatched name falls through to your account default category. Pre-classify products (with AI assistance) via create_product for a reviewed, deterministic result.' },
              productCode: { type: 'string', description: 'Optional SKU or product code — the primary catalogue lookup key. When provided, the classification result is cached per account so the same product is not re-classified on every transaction.' },
              taxCategory: { type: 'string', description: 'Optional explicit category slug (e.g. saas_business, digital_general, physical_goods_general, professional_services). Skips catalogue lookup entirely.' },
